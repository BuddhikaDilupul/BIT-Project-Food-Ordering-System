...
const Customers = () => {
  ...
  const handleLoadCustomers = async () => {
    try {
      const _data = await fetch(`http://localhost:4041/api/customer/list`, {
        method: 'GET',
        headers: {
  ...
  const search = async () => {
    clearData()
    try {
      const _data = await fetch(`http://localhost:4041/api/search/customer`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
  ...
  const handleDeleteData = async (props) => {
    try {
      ...
      const _data = await fetch('http://localhost:4041/api/customer/delete/' + id, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
