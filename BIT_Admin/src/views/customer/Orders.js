...
const Orders = (props) => {
  ...
  const handleLoadOrders = async () => {
    try {
      const _data = await fetch(`http://localhost:4041/api/reports/` + id, {
        method: 'GET',
        headers: {
  ...
  const search = async () => {
    ...
    try {
      ...
      _data = await fetch(`http://localhost:4041/api/search/order`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
  ...
  // console.log(display, 'llll')
  const handleDeleteData = async (props) => {
    try {
      const id = props

      const _data = await fetch('http://localhost:4041/api/order/delete/' + id, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
