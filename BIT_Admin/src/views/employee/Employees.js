...
const Employees = () => {
  ...
  const handleLoadMembers = async () => {
    try {
      const _data = await fetch(`http://localhost:4041/api/staff/listEmployee`, {
        method: 'GET',
        headers: {
  ...
  const search = async () => {
    clearData()
    try {
      const _data = await fetch(`http://localhost:4041/api/search/employee`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
  ...
  const handleDeleteData = async (props) => {
    try {
      ...
      const _data = await fetch('http://localhost:4041/api/staff/delete/' + id, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
