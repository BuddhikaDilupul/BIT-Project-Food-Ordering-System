...
const Locations = () => {
  ...
  const handleLoadData = async () => {
    try {
      const _data = await fetch(`http://localhost:4041/api/delivery/`, {
        method: 'GET',
        headers: {
  ...
  const search = async () => {
    clearData()
    try {
      const _data = await fetch(`http://localhost:4041/api/search/delivery`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
  ...
  const handleDeleteData = async (props) => {
    try {
      ...
      const _data = await fetch('http://localhost:4041/api/delivery/delete/' + id, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + localStorage.getItem('authToken'),
