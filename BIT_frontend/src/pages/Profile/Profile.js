...
const Profile = () => {
  ...
  const TokenCheck = async () => {
    ...
    if (token) {
      try {
        const _data = await fetch(`http://localhost:4041/api/authtoken`, {
          method: 'GET',
          headers: {
  ...
  const loadData = async () => {
    const token = localStorage.getItem('authToken')
    try {
      const _data = await fetch(`http://localhost:4041/api/customer/`, {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
          Authorization: 'Bearer ' + token,
  ...
  const changePassword = async (data) => {
    if (data) {
      try {
        const _data = await fetch('http://localhost:4041/api/customer/reset', {
          method: 'PUT',
          headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + token,
