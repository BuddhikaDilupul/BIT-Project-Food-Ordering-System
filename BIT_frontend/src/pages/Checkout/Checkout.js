...
function Checkout() {
  ...
  const clearCart = async () => {
    try {
      ...
      const _cartList = await fetch(`http://localhost:4041/api/cart/clear`, {
        method: 'GET',
        headers: {
  ...
  const LoadOrderItem = async () => {
    try {
      const _deliveryLocationDetails = await fetch(
        `http://localhost:4041/api/delivery/location`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
      ...
      const _customerDetails = await fetch(
        `http://localhost:4041/api/customer/`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  const onSubmit = async (data) => {
    ...
    if (data) {
      ...
      try {
        setLoadingButton(true)
        const _data = await fetch('http://localhost:4041/api/order', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            Authorization: 'Bearer ' + token,
